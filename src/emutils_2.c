 ...
 */
 ...
 /* Added function EmissionMatrix_mv_reduce_pre_gpu, modified from EmissionMatrix_mv_reduce function to prepare arrays for calculation on gpu: 
 Do cholesky decomp (i.e. sigma = LL') on cpu and compute emission matrix later on gpu
 */
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
/* calculate scores on single TF: input TF_index < phmm->M; return scores into a gsl_vector */
void CalMotifScore_P_single_TF(HMM *phmm, gsl_vector * S, int *O1, int P, int *peakPos, int TF_index) 
{
  int k, t, j, i, n, m, D;
  int start, end;
  double tempF, tempR, tmpL, tmpR, tmp;
...
#pragma omp for /* Do the calculation in parallel, peak by peak */
    for (k = 0; k < P; k++) {
      ...
      for (t = start - 1; t < end; t++) {
        if (t < (start + D - 1)){
          gsl_vector_set (S, t, tmpL);
        }
        else if (t > (end - D - 1)) {
          gsl_vector_set (S, t, tmpR);
        }
        else{
          tempF = 0.0;
          tempR = 0.0;
          ...
          gsl_vector_set(tempList, 0, MAX(tempF, tempR));
          gsl_vector_set (S, t, gsl_vector_max(tempList));
        }
      }
      gsl_vector_free(tempList);  //TODO:change in old file
    }
...
/*calculate motif score on filtered P_i peaks for TFs [TF_start,TF_end); S: K (>=M) x totalT_i */
void CalMotifScore_P_partial(HMM *phmm, gsl_matrix * S, int *O1, int P_i, int *peakPos, int *peakPos_i, int *posIndex_i, int TF_start, int TF_end)
{
  int k, t, j, i, n, m, D;
  int start, end;
  double tempF, tempR, tmpL, tmpR, tmp;
  gsl_vector * tempList;
  for (m = TF_start; m < TF_end; m++) {
    D = phmm->D[m]; /* length of motif */
#pragma omp parallel num_threads(THREAD_NUM) \
  private(start, end, tempF, tempR, tmpL, tmpR, tmp, t, i, j, n, tempList)
  {
#pragma omp for /* Do the calculation in parallel, peak by peak */
    for (k = 0; k < P_i; k++) {
      tempList = gsl_vector_alloc(D);
      start = peakPos[posIndex_i[k]];
      end = peakPos[posIndex_i[k]+1] - 1; /* The starting and ending position for fitered peak k */
      ...
      for (t = start - 1; t < end; t++) {
        if (t < (start + D - 1)){
          gsl_matrix_set (S, m, t -start + peakPos_i[k], tmpL); //new coordinate t'=t -start + peakPos_i[k] mapping to totalT_i seq
        }
        else if (t > (end - D - 1)) {
          gsl_matrix_set (S, m, t -start + peakPos_i[k], tmpR);
        }
        else{
          tempF = 0.0;
          tempR = 0.0;
          ...
          gsl_vector_set(tempList, 0, MAX(tempF, tempR));
          gsl_matrix_set (S, m, t -start + peakPos_i[k], gsl_vector_max(tempList));
        }
      }
      gsl_vector_free(tempList);  //TODO:change in old file
    }
...
void EmissionMatrix_mv_reduce_pre_gpu(HMM* phmm, gsl_matrix * obs_matrix, int P, int *peakPos, int T,
                                      gsl_matrix ** cov_matrix_tmp,gsl_vector ** deleted_vector) //only do sigma=LL' part before input to gpu
{
  int K = phmm->K; //for easier refering
  int N = phmm->N;
  int thread_id;
  int t, i, j, m;
  ...
  if (phmm->K == 1) {
    ...
    //gsl_matrix *cov_matrix_tmp[phmm->N]; //will be L after cholesky_decomp, 
    gsl_matrix *tmp_matrix; //tmp matrix for cov_matrix_tmp
    gsl_vector *tmp_vector2; //tmp vector for deleted_vector
    //gsl_vector *deleted_vector[phmm->N]; //record error_rows
    const int FULL = -1;
    const int EMPTY = -2;
    int n;
    int l;
...
#pragma omp for
      for (i = 0; i < phmm->N; i++) {
        ...
        do {
          ...
          //printf("error_row: %d\tstate:%d\tn:%d\n",error_row,i,n);
